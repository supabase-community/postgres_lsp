...
impl ConfigName {
    const PGLSP_TOML: [&'static str; 1] = ["pglsp.toml"];

    pub const fn pglsp_toml() -> &'static str {
        Self::PGLSP_TOML[0]
    }
                ...
                match self.read_file_from_path(&file_path) {
                    Ok(content) => {
                        if is_searching_in_parent_dir {
                            info!(
                                "Auto discovered the file at the following path that isn't in the working directory:\n{:?}",
...
pub trait TraversalContext: Sync {
    ...
    fn can_handle(&self, path: &PgLspPath) -> bool;
    ...
    fn record(&self, visitor: &mut dyn Visit) -> io::Result<()> {
        match self {
			ErrorKind::CantReadFile(path) => visitor.record_log(
		        LogCategory::Error,
			    &format!("Can't read the following file, maybe for permissions reasons or it doesn't exist: {path}")
			),

            ErrorKind::UnknownFileType => visitor.record_log(
                LogCategory::Info,
                &"Encountered a file system entry that's neither a file, directory or symbolic link",
            ),
            ErrorKind::DereferencedSymlink(path) => visitor.record_log(
                LogCategory::Info,
                &format!("Encountered a file system entry that is a broken symbolic link: {path}"),
            ),
            ErrorKind::DeeplyNestedSymlinkExpansion(path) => visitor.record_log(
                LogCategory::Error,
                &format!("Encountered a file system entry with too many nested symbolic links, possibly forming an infinite cycle: {path}"),
            ),
        }
    }
}
