...
pub fn resolve_from_enriched_ast(pos: TextSize, ast: &pg_syntax::AST) -> Option<Hoverable> {
    let node = ast.covering_node(TextRange::empty(pos))?;

    match node.node {
        pg_query_ext::NodeEnum::RangeVar(ref range_var) => {
            Some(Hoverable::Relation(HoverableRelation {
                range: node.range(),
