...
impl Change {
    ...
    pub fn apply(&self, doc: &mut Document) -> Vec<StatementChange> {
        ...
        if self.range.is_none() {
            ...
            doc.statement_ranges = pglt_statement_splitter::split(&self.text).ranges.to_vec();
            ...
            .position(|r| r.contains_range(self.range.unwrap()))
        {
            ...
            for range in pglt_statement_splitter::split(extracted_text).ranges {
                match doc
                    .statement_ranges
...
mod tests {
    ...
    fn test_document_apply_changes() {
        ...
        for r in &pglt_statement_splitter::split(&d.text).ranges {
            assert!(
