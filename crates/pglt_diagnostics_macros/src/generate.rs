...
fn generate_struct_diagnostic(input: DeriveStructInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params pglt_diagnostics::Diagnostic for #ident #generics {
            #category
            #severity
            #description
            #message
...
fn generate_category(input: &DeriveStructInput) -> TokenStream {
    let category = match &input.category {
        Some(StaticOrDynamic::Static(value)) => quote! {
            pglt_diagnostics::category!(#value)
        },
    ...
    quote! {
        fn category(&self) -> Option<&'static pglt_diagnostics::Category> {
            Some(#category)
        }
    }
}
...
fn generate_severity(input: &DeriveStructInput) -> TokenStream {
    let severity = match &input.severity {
        Some(StaticOrDynamic::Static(value)) => quote! {
            pglt_diagnostics::Severity::#value
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
        },
    ...
    quote! {
        fn severity(&self) -> pglt_diagnostics::Severity {
            #severity
        }
    }
}
...
fn generate_description(input: &DeriveStructInput) -> TokenStream {
    let description = match &input.description {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            ...
            let write = pglt_diagnostics::termcolor::NoColor::new(&mut buffer);
            let mut write = pglt_diagnostics::console::fmt::Termcolor(write);
            let mut write = pglt_diagnostics::console::fmt::Formatter::new(&mut write);

            use pglt_diagnostics::console as pglt_console;
            write.write_markup(&pglt_diagnostics::console::markup!{ #markup })
...
fn generate_message(input: &DeriveStructInput) -> TokenStream {
    let message = match &input.message {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            use pglt_diagnostics::console as pglt_console;
            fmt.write_markup(pglt_diagnostics::console::markup!{ #markup })
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            pglt_diagnostics::console::fmt::Display::fmt(&self.#value, fmt)
        },
        None => return quote!(),
    };

    quote! {
        fn message(&self, fmt: &mut pglt_diagnostics::console::fmt::Formatter<'_>) -> ::std::io::Result<()> {
            #message
        }
    }
}
...
fn generate_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn advices(&self, visitor: &mut dyn pglt_diagnostics::Visit) -> ::std::io::Result<()> {
            #( pglt_diagnostics::Advices::record(&self.#advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_verbose_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn verbose_advices(&self, visitor: &mut dyn pglt_diagnostics::Visit) -> ::std::io::Result<()> {
            #( pglt_diagnostics::Advices::record(&self.#verbose_advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_location(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn location(&self) -> pglt_diagnostics::Location<'_> {
            pglt_diagnostics::Location::builder()
                #( .#method(&self.#field) )*
                .build()
        }
    }
...
fn generate_tags(input: &DeriveStructInput) -> TokenStream {
    let tags = match &input.tags {
        Some(StaticOrDynamic::Static(value)) => {
            ...
            quote! {
                #( pglt_diagnostics::DiagnosticTags::#values )|*
            }
        }
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
    ...
    quote! {
        fn tags(&self) -> pglt_diagnostics::DiagnosticTags {
            #tags
        }
    }
}
...
fn generate_source(input: &DeriveStructInput) -> TokenStream {
    match &input.source {
        Some(value) => quote! {
            fn source(&self) -> Option<&dyn pglt_diagnostics::Diagnostic> {
                self.#value.as_deref()
            }
        },
        None => quote!(),
...
fn generate_enum_diagnostic(input: DeriveEnumInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params pglt_diagnostics::Diagnostic for #ident #generics {
            fn category(&self) -> Option<&'static pglt_diagnostics::Category> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::category(error),)*
                }
            }

            fn description(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::description(error, fmt),)*
                }
            }

            fn message(&self, fmt: &mut pglt_console::fmt::Formatter<'_>) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::message(error, fmt),)*
                }
            }
            ...
            fn tags(&self) -> pglt_diagnostics::DiagnosticTags {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::tags(error),)*
                }
            }

            fn location(&self) -> pglt_diagnostics::Location<'_> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::location(error),)*
                }
            }

            fn source(&self) -> Option<&dyn pglt_diagnostics::Diagnostic> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::source(error),)*
                }
            }

            fn advices(&self, visitor: &mut dyn pglt_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::advices(error, visitor),)*
                }
            }

            fn verbose_advices(&self, visitor: &mut dyn pglt_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pglt_diagnostics::Diagnostic::verbose_advices(error, visitor),)*
                }
            }
        }
    }
