use pglt_analyse::{GroupCategory, RegistryVisitor, Rule, RuleCategory, RuleFilter, RuleGroup};
use pglt_configuration::RuleSelector;
...
impl<'a, 'b> AnalyserVisitorBuilder<'a, 'b> {
    ...
    pub(crate) fn finish(self) -> (Vec<RuleFilter<'a>>, Vec<RuleFilter<'a>>) {
        ...
        if let Some(mut lint) = self.lint {
            pglt_analyser::visit_registry(&mut lint);
