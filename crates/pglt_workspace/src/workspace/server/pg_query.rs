...
use dashmap::DashMap;
use pglt_diagnostics::serde::Diagnostic as SDiagnostic;
use pglt_query_ext::diagnostics::*;

use super::{change::ModifiedStatement, document::Statement};

pub struct PgQueryStore {
    ast_db: DashMap<Statement, Arc<pglt_query_ext::NodeEnum>>,
...
impl PgQueryStore {
    ...
    pub fn get_ast(&self, statement: &Statement) -> Option<Arc<pglt_query_ext::NodeEnum>> {
        self.ast_db.get(statement).map(|x| x.clone())
    }

    pub fn add_statement(&self, statement: &Statement, content: &str) {
        let r = pglt_query_ext::parse(content);
        if let Ok(ast) = r {
    ...
    pub fn get_diagnostics(&self, stmt: &Statement) -> Vec<pglt_diagnostics::serde::Diagnostic> {
        self.diagnostics
            .get(stmt)
