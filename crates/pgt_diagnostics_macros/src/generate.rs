...
fn generate_struct_diagnostic(input: DeriveStructInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params pgt_diagnostics::Diagnostic for #ident #generics {
            #category
            #severity
            #description
            #message
...
fn generate_category(input: &DeriveStructInput) -> TokenStream {
    let category = match &input.category {
        Some(StaticOrDynamic::Static(value)) => quote! {
            pgt_diagnostics::category!(#value)
        },
    ...
    quote! {
        fn category(&self) -> Option<&'static pgt_diagnostics::Category> {
            Some(#category)
        }
    }
}
...
fn generate_severity(input: &DeriveStructInput) -> TokenStream {
    let severity = match &input.severity {
        Some(StaticOrDynamic::Static(value)) => quote! {
            pgt_diagnostics::Severity::#value
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
        },
    ...
    quote! {
        fn severity(&self) -> pgt_diagnostics::Severity {
            #severity
        }
    }
}
...
fn generate_description(input: &DeriveStructInput) -> TokenStream {
    let description = match &input.description {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            ...
            let write = pgt_diagnostics::termcolor::NoColor::new(&mut buffer);
            let mut write = pgt_diagnostics::console::fmt::Termcolor(write);
            let mut write = pgt_diagnostics::console::fmt::Formatter::new(&mut write);

            use pgt_diagnostics::console as pgt_console;
            write.write_markup(&pgt_diagnostics::console::markup!{ #markup })
...
fn generate_message(input: &DeriveStructInput) -> TokenStream {
    let message = match &input.message {
        ...
        Some(StaticOrDynamic::Static(StringOrMarkup::Markup(markup))) => quote! {
            use pgt_diagnostics::console as pgt_console;
            fmt.write_markup(pgt_diagnostics::console::markup!{ #markup })
        },
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            pgt_diagnostics::console::fmt::Display::fmt(&self.#value, fmt)
        },
        None => return quote!(),
    };

    quote! {
        fn message(&self, fmt: &mut pgt_diagnostics::console::fmt::Formatter<'_>) -> ::std::io::Result<()> {
            #message
        }
    }
}
...
fn generate_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn advices(&self, visitor: &mut dyn pgt_diagnostics::Visit) -> ::std::io::Result<()> {
            #( pgt_diagnostics::Advices::record(&self.#advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_verbose_advices(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn verbose_advices(&self, visitor: &mut dyn pgt_diagnostics::Visit) -> ::std::io::Result<()> {
            #( pgt_diagnostics::Advices::record(&self.#verbose_advices, visitor)?; )*
            Ok(())
        }
    }
}
...
fn generate_location(input: &DeriveStructInput) -> TokenStream {
    ...
    quote! {
        fn location(&self) -> pgt_diagnostics::Location<'_> {
            pgt_diagnostics::Location::builder()
                #( .#method(&self.#field) )*
                .build()
        }
    }
...
fn generate_tags(input: &DeriveStructInput) -> TokenStream {
    let tags = match &input.tags {
        Some(StaticOrDynamic::Static(value)) => {
            ...
            quote! {
                #( pgt_diagnostics::DiagnosticTags::#values )|*
            }
        }
        Some(StaticOrDynamic::Dynamic(value)) => quote! {
            self.#value
    ...
    quote! {
        fn tags(&self) -> pgt_diagnostics::DiagnosticTags {
            #tags
        }
    }
}
...
fn generate_source(input: &DeriveStructInput) -> TokenStream {
    match &input.source {
        Some(value) => quote! {
            fn source(&self) -> Option<&dyn pgt_diagnostics::Diagnostic> {
                self.#value.as_deref()
            }
        },
        None => quote!(),
...
fn generate_enum_diagnostic(input: DeriveEnumInput) -> TokenStream {
    ...
    quote! {
        impl #generic_params pgt_diagnostics::Diagnostic for #ident #generics {
            fn category(&self) -> Option<&'static pgt_diagnostics::Category> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::category(error),)*
                }
            }

            fn description(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::description(error, fmt),)*
                }
            }

            fn message(&self, fmt: &mut pgt_console::fmt::Formatter<'_>) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::message(error, fmt),)*
                }
            }

            fn severity(&self) -> pgt_diagnostics::Severity {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::severity(error),)*
                }
            }

            fn tags(&self) -> pgt_diagnostics::DiagnosticTags {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::tags(error),)*
                }
            }

            fn location(&self) -> pgt_diagnostics::Location<'_> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::location(error),)*
                }
            }

            fn source(&self) -> Option<&dyn pgt_diagnostics::Diagnostic> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::source(error),)*
                }
            }

            fn advices(&self, visitor: &mut dyn pgt_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::advices(error, visitor),)*
                }
            }

            fn verbose_advices(&self, visitor: &mut dyn pgt_diagnostics::Visit) -> std::io::Result<()> {
                match self {
                    #(Self::#variants(error) => pgt_diagnostics::Diagnostic::verbose_advices(error, visitor),)*
                }
            }
        }
    }
